...
condA = cond(A); % se calcula el n�mero de condici�n de A
x = A\b; % soluci�n original
A(3,1) = A(3,1) + 0.01; % se le suma una cent�sima al primer elemento de la tercera columna.
x1 = A\b; % soluci�n modificada
...
T = 1e-8; N = 500; % par�metros que se le pasan a la funci�n de Jacobi.

%%% �Es aplicable el m�todo?
...
else, disp("No se puede aplicar el m�todo a esta matriz."), exit, end

%%% Se aplica el m�todo de Jacobi una vez confirmado.
x = jacobi(A, b, T, N);
...
T = 1e-6; N = 100; % par�metros que se le pasan a la funci�n newton_n

% Se declaran las variables y las funciones simb�licas.
syms A B C
f1(A,B,C) = A*sin(t1*B+C) - y1; 
f2(A,B,C) = A*sin(t2*B+C) - y2; 
f3(A,B,C) = A*sin(t3*B+C) - y3;
f(A,B,C) = [ f1(A,B,C) ; f2(A,B,C) ; f3(A,B,C) ]; % Se declara la funci�n f como un vector de ecuaciones (no lineales).
df = jacobian(f); % Se halla su matriz jacobiana.

% Se convierten ambas funciones a num�ricas vectoriales.
...
% Se declaran las funciones an�nimas.
a1 = @(x,y) -exp(x*x) + 8*x*sin(y-30);
a2 = @(x,y) x*x + y*y + 2.582*y + 0.666681;
...
T = 1e-7; N = 500; % par�metros de la funci�n iterar.
res = iterar(A, b, x0, T, N); % Se calcula el resultado mediante la funci�n iterar.m
